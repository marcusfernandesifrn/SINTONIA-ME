...
def _fig_ampere_contorno():
    ...
    outer=mpatches.FancyBboxPatch((-2.2,-1.5),4.4,3.,boxstyle="round,pad=.3",
                                   lw=1.5,ec=CZ,fc="#1a2233",zorder=2)
    inner=mpatches.FancyBboxPatch((-1.,-.7),2.,1.4,boxstyle="round,pad=.1",
                                   lw=1.5,ec=CZ,fc="#0e1117",zorder=3)
    ...
    ax.text(0,-2.1,r"$\oint_C \vec{H}\cdot d\vec{l} = N\,i$",
            ...
            bbox=dict(boxstyle="round,pad=.3",fc="#1a2233",ec=AZ,alpha=.8))
...
def _fig_campo_r():
    ...
    ax.text(1.7, .4,r"$\oint \vec{H}\cdot d\vec{l} = H\cdot 2\pi r = i$",color=BR,fontsize=11)
...
def _fig_BH():
    ...
    ax.plot(H,B_lin/B_lin.max()*.25,"--",color=CI,lw=1.8,label="Ar / cobre ($\\mu_r=1$)")
    Bsat=1.8; mur=3500
    B_fe=Bsat*(1-np.exp(-mur*4*np.pi*1e-7*H/Bsat))
    ax.plot(H,B_fe,color=AZ,lw=2.2,label="Ferromagnético ($\\mu_r\\approx 2000$–$6000$)")
    idx=60
    ...
    ax.legend(fontsize=9,facecolor="#1a2233",edgecolor=CZ,labelcolor=BR)
...
def _fig_circuito_mag():
    fig, ax = plt.subplots(figsize=(6.5,4.5))
    fig.patch.set_facecolor("#0e1117"); ax.set_facecolor("#0e1117")
    ax.set_xlim(-3.5,3.5); ax.set_ylim(-2.5,2.5); ax.set_aspect("equal"); ax.axis("off")
    ...
    for ang in np.linspace(np.pi*.25,np.pi*.75,9):
        ...
        ax.add_patch(mpatches.Ellipse((xc,yc),.35,.18,angle=np.degrees(ang)+90,
    ...
    ax.text(0,-2.3,
            r"$\mathcal{F}=N\,i$   $\mathcal{R}=\frac{l}{\mu A}$   $\phi=\frac{\mathcal{F}}{\mathcal{R}}$",
            color=BR,fontsize=10,ha="center",va="top",
            bbox=dict(boxstyle="round,pad=.4",fc="#1a2233",ec=AZ,alpha=.85))
...
def _fig_analogia():
    ...
    for col,ax in enumerate(axes):
        ...
        for k,(nome,el,em) in enumerate(linhas):
            ...
            ax.text(.08,y,f"{nome}:",color=CZ,fontsize=9,ha="left",va="center",
                    transform=ax.transAxes)
            ax.text(.58,y,val,color=BR,fontsize=10,ha="center",va="center",
                    transform=ax.transAxes,
        ...
        ax_ins.set_facecolor("#0e1117"); ax_ins.axis("off")
        with schemdraw.Drawing(canvas=ax_ins) as d:
            d.config(fontsize=9,color=BR)
            label="$e$" if col==0 else "$\\mathcal{F}$"
            ec=VD if col==0 else AZ
            V=d.add(elm.SourceV().label(label,loc="left").color(ec))
...
def _fig_entreferro():
    ...
    fig.patch.set_facecolor("#0e1117")
    # — esboço físico —
    ax=axes[0]; ax.set_facecolor("#0e1117"); ax.axis("off")
    ...
    ax.add_patch(Rectangle((4.5,6.4),1.,1.,color="#0e1117",ec=LR,lw=1.5,ls="--",zorder=3))
    ax.text(5.,6.1,"Entreferro\n$\\ell_g$",color=LR,fontsize=7,ha="center",va="top")
    for y0 in np.linspace(2.6,5.8,7):
        ax.add_patch(mpatches.Ellipse((1.75,y0),.85,.35,color=AZ,zorder=4,alpha=.8))
    ax.text(.4,4.2,"$N$\nespiras",color=AZ,fontsize=8,ha="center")
    # fluxo
    xs=[2.25,2.25,5.,7.75,7.75,5.,4.7]; ys=[4.1,6.9,6.9,6.9,4.1,4.1,4.1]
...
def _fig_frangeamento():
    ...
    ax.add_patch(plt.Circle((0,0),2.2,color="#0e1117",ec=CZ,lw=1.,zorder=3))
    ax.add_patch(plt.Circle((0,0),1.9,color="#1a3055",ec=CZ,lw=1.2,zorder=4))
    ax.add_patch(plt.Circle((0,0),1.1,color="#0e1117",ec=CZ,lw=.8,zorder=5))
    ax.add_patch(plt.Circle((0,0),.25,color=CZ,zorder=6))
    t=np.linspace(0,2*np.pi,200)
    ax.fill_between(2.2*np.cos(t),2.2*np.sin(t),1.9*np.cos(t),1.9*np.sin(t),
                    color=LR,alpha=.25,zorder=3)
...
def _fig_indutancia():
    ...
    for y0 in np.linspace(2.5,6.,8):
        ax.add_patch(mpatches.Ellipse((1.75,y0),1.,.4,color=AZ,zorder=4,alpha=.8))
    ax.text(.5,4.2,"$N$",color=AZ,fontsize=14,ha="center")
    ax.plot([2.5,2.5,7.5,7.5,5.],[4.,6.5,6.5,4.,4.],color=VD,lw=2,alpha=.8)
    ...
    ax.text(5.,.3,
            ...
            bbox=dict(boxstyle="round,pad=.35",fc="#1a2233",ec=AZ,alpha=.85))
    ax2=axes[1]; ax2.set_facecolor("#0e1117"); ax2.axis("off")
    ...
    for k,(eq,desc) in enumerate(eqs):
        ...
        ax2.text(.5,y,eq,color=BR,fontsize=11,ha="center",va="center",
                 ...
                 bbox=dict(boxstyle="round,pad=.3",fc="#1a2233",ec=AZ,alpha=.7))
...
def _fig_indutancia_mutua():
    ...
    for y0 in np.linspace(2.5,5.8,7):
        ax.add_patch(mpatches.Ellipse((1.25,y0),1.,.38,color=AZ,zorder=4,alpha=.85))
    ax.text(.05,4.1,"$N_1\\ i_1$",color=AZ,fontsize=9,ha="left")
    for y0 in np.linspace(2.5,5.8,7):
        ax.add_patch(mpatches.Ellipse((8.75,y0),1.,.38,color=VD,zorder=4,alpha=.85))
    ax.text(9.6,4.1,"$N_2\\ i_2$",color=VD,fontsize=9,ha="right")
    ax.plot([2.,2.,8.,8.,5.],[4.1,6.5,6.5,4.1,4.1],color=RX,lw=2,alpha=.8)
...
def _fig_histerese():
    ...
    Hp=np.linspace(-Hmax,Hmax,500); Hn=np.linspace(Hmax,-Hmax,500)
    for i,(ax,title) in enumerate(zip(axes,["Laço de Histerese",
        ...
        if i==0:
            ...
            ax.annotate("$H_c$",xy=(200,0),xytext=(350,.45),color=LR,fontsize=8,
                        arrowprops=dict(arrowstyle="->",color=LR))
            ax.legend(fontsize=8,facecolor="#1a2233",edgecolor=CZ,labelcolor=BR,loc="lower right")
        else:
            cores=[CI,AZ,RX,LR]
            for k,Ha in enumerate([250,500,750,Hmax]):
                Hp2=np.linspace(-Ha,Ha,400); Hn2=np.linspace(Ha,-Ha,400)
                hc2=200*(Ha/Hmax)**.5
...
def _fig_parasita():
    ...
    ax2.legend(fontsize=8,facecolor="#1a2233",edgecolor=CZ,labelcolor=BR)
    ax2.tick_params(colors=CZ)
    for sp in ax2.spines.values(): sp.set_edgecolor("#444")
    ax2.set_facecolor("#0e1117")
    ax2.text(180,.07,
...
def _exp_circuito():
    ...
    for ax_,xs,ys,xl,yl,t_,xv,yv in [
        (axes[0],i_a,phi_a*1e3,"$i$ (A)","$\\phi$ (mWb)","Fluxo vs. Corrente",i,phi*1e3),
        (axes[1],Hc*i_a/max(i,1e-9),B*i_a/max(i,1e-9),"$H$ (A/m)","$B$ (T)","Ponto B-H (linear)",Hc,B),
    ]:
        ...
        ax_.set_title(t_,color=BR,fontsize=9)
...
def _exp_BH():
    ...
    if show_hist:
        Hc2=H_op*.15
        ax.plot(H_a,Bmag(H_a-Hc2,mur_max*.8,Bsat),"--",color=RX,lw=1.4,alpha=.7,label="Laço superior")
        ax.plot(H_a,Bmag(H_a+Hc2,mur_max*.8,Bsat),"--",color=LR,lw=1.4,alpha=.7,label="Laço inferior")
    ...
    ax.legend(fontsize=8,facecolor="#1a2233",edgecolor=CZ,labelcolor=BR)
    ax.tick_params(colors=CZ)
    for sp in ax.spines.values(): sp.set_edgecolor("#444")
    ax2=axes[1]; ax2.set_facecolor("#0e1117")
    ...
    ax2.plot(H_op,mr_op,"o",color=LR,ms=9,zorder=5,label=f"$\\mu_r$ local={mr_op:.0f}")
    ...
    fig.tight_layout(); _show(fig)
    colr=st.columns(4)
    for c,(lab,val) in zip(colr,[("$B$ (T)",f"{B_op:.4f}"),
...
def run():
    ...
    st.markdown('<div class="mod-sub">MOD 01 &nbsp;·&nbsp; Introdução à Conversão Eletromecânica de Energia</div>',
...
""")
        ...
        st.markdown("""
Para um **condutor isolado** a distância $r$, onde $\\vec{H} \\parallel d\\vec{l}$ e $|H|$ constante:
""")
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        st.latex(r"H \cdot 2\pi r = i \quad\Rightarrow\quad H = \frac{i}{2\pi r}")
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown("""
Considerando a existência de um ângulo $\\theta$ entre os vetores $\\vec{H}$ e $d\\vec{l}$:
""")
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        st.latex(r"\oint \vec{H} \cdot d\vec{l} = \oint H\, dl\, \cos\theta = N\,i")
        st.markdown('</div>', unsafe_allow_html=True)
    with c2:
        _show(_fig_regra_mao(), "Campo H ao redor de um condutor (regra da mão direita)")

    c1,c2=st.columns([1,1])
    with c1:
        _show(_fig_campo_r(), "Campo H a distância r de um condutor isolado")
    with c2:
        _show(_fig_ampere_contorno(), "Lei de Ampère — contorno fechado num núcleo toroidal")
    ...
    with c1:
        st.markdown("""
A **densidade de fluxo magnético** $B$ (T = Wb/m²) está relacionada à intensidade de campo $H$ pela
...
- $\\mu_r$: permeabilidade relativa do meio (adimensional)
""")
...
""")
        ...
        st.markdown("""
⚠️ Para materiais ferromagnéticos, $\\mu_r$ **não é constante**: varia com $H$
(curva de magnetização não-linear) e com a história magnética do material (histerese).
A análise linear é válida apenas na região inicial da curva $B$-$H$.
""")
        st.markdown('</div>', unsafe_allow_html=True)
    with c2:
        _show(_fig_BH(), "Curva B-H: material linear vs. ferromagnético")
    ...
    st.markdown("""
Combinando Ampère ($H\\,\\ell = N\\,i$), a relação $B = \\mu H$ e a definição $\\phi = B\\,A$,
obtém-se a **lei do circuito magnético** — análoga à lei de Ohm:
""")
    c1,c2=st.columns([1,1])
    with c1:
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        st.latex(r"\mathcal{F} = N\,i \quad \text{(FMM — força magnetomotriz, A·t)}")
        st.latex(r"\mathcal{R} = \frac{\ell}{\mu\,A} \quad \text{(relutância, A·t/Wb)}")
        st.latex(r"\phi = \frac{\mathcal{F}}{\mathcal{R}} = \frac{N\,i\,\mu\,A}{\ell} \quad \text{(Wb)}")
        st.markdown('</div>', unsafe_allow_html=True)
        _show(_fig_circuito_mag(), "Núcleo toroidal: φ, ℱ e ℛ")
    with c2:
        _show(_fig_analogia(), "Analogia: circuito elétrico ↔ circuito magnético")
        st.markdown('<div class="def-box">', unsafe_allow_html=True)
        st.markdown("""
...
""")
    ...
    with c1:
        st.markdown("""
Quando o circuito inclui um trecho de ar (entreferro $\\ell_g$), as relutâncias somam-se em série:
""")
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        st.latex(r"N\,i = (\mathcal{R}_c + \mathcal{R}_g)\,\phi")
        st.latex(r"\mathcal{R}_c = \frac{\ell_c}{\mu_r\mu_0 A_c}, \quad \mathcal{R}_g = \frac{\ell_g}{\mu_0 A_g}")
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown('<div class="nota-box">', unsafe_allow_html=True)
        st.markdown("""
⚠️ **Dominância do entreferro:** com $\\mu_r=2000$, um entreferro de 1 mm é equivalente
a 2 m de núcleo ferromagnético de mesma seção — o entreferro domina a relutância total.
""")
        st.markdown('</div>', unsafe_allow_html=True)
    with c2:
        _show(_fig_entreferro(), "Circuito magnético com entreferro")

    st.markdown('<div class="subsec">3.2 · Máquina Rotativa e Frangeamento</div>', unsafe_allow_html=True)
    c1,c2=st.columns([1,1])
    with c1:
        st.markdown("""
Em máquinas rotativas, o **entreferro** separa estator e rotor.
O **efeito de frangeamento** (_fringing_) alarga as linhas de campo ao atravessar o ar,
aumentando a área efetiva: $A_g > A_c$.

Correção prática para polo retangular ($a\\times b$):
""")
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        st.latex(r"A_g \approx (a + \ell_g)(b + \ell_g)")
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown("""
Para $\\ell_g \\ll a, b$, o frangeamento é desprezível.
Em máquinas reais, utiliza-se a relação:
""")
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        st.latex(r"B_c = B_g \quad;\quad A_g = A_c \quad \Rightarrow \quad \mathcal{R}_g = \frac{\ell_g}{\mu_0 A_c}")
        st.markdown('</div>', unsafe_allow_html=True)
    with c2:
        _show(_fig_frangeamento(), "Máquina rotativa e frangeamento no entreferro")
    ...
    with c1:
        st.markdown("""
O **fluxo concatenado** $\\lambda$ (Wb·t) é o fluxo total linkado com todas as espiras:
""")
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        st.latex(r"\lambda = N\,\phi = \frac{N^2}{\mathcal{R}}\,i = L\,i")
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown("A **indutância** $L$ (H = Wb/A) é:")
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        st.latex(r"L = \frac{\lambda}{i} = \frac{N^2}{\mathcal{R}} = \frac{N^2\,\mu\,A}{\ell}")
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown("A **lei de Faraday** fornece a FEM induzida:")
        st.markdown('<div class="eq-box">', unsafe_allow_html=True)
        ...
        st.markdown('</div>', unsafe_allow_html=True)
    with c2:
        _show(_fig_indutancia(), "Indutância, Lei de Faraday e energia armazenada")

    st.markdown('<div class="subsec">4.1 · Indutância Mútua e Energia Armazenada</div>',
                unsafe_allow_html=True)
    c1,c2=st.columns([1,1])
    with c1:
        _show(_fig_indutancia_mutua(), "Indutância mútua entre duas bobinas")
    with c2:
...
- Sistema linear: $\\mu$ constante (região linear da curva $B$-$H$)
...
""")
        ...
        st.markdown('<div class="nota-box">', unsafe_allow_html=True)
        st.markdown("""
Nas máquinas elétricas, a variação de $W_L$ com a posição do rotor
é o mecanismo de **geração de força e torque eletromagnético**.
""")
        st.markdown('</div>', unsafe_allow_html=True)

    # ── 5. Histerese ─────────────────────────────────────────────────────────
    st.markdown('<div class="sec-title">5 · Histerese Magnética</div>',
                unsafe_allow_html=True)

    c1,c2=st.columns([1.1,1])
    with c1:
        _show(_fig_histerese(), "Laço de histerese B-H e laços para diferentes amplitudes")
    with c2:
        st.markdown("""
A **histerese** é o fenômeno pelo qual o estado magnético depende da história
de magnetização do material, não apenas do campo atual $H$.
...
""")
    ...
    c1,c2=st.columns([1.1,1])
    with c1:
        _show(_fig_parasita(), "Correntes parasitas, laminação e perdas no núcleo")
    with c2:
        st.markdown("""
**Correntes parasitas** (_Eddy currents_) são correntes induzidas no núcleo condutor
pela variação de $B$. Dissipam energia como calor.

**Redução:**
- Material de **alta resistividade** (ferrite, aço silício com 3–4% Si)
- **Laminação**: chapas finas isoladas interrompem as correntes — $P_e \\propto d^2$
...
""")
        ...
        st.markdown('<div class="nota-box">', unsafe_allow_html=True)
        st.markdown("""
Em máquinas e transformadores, as perdas no núcleo são relevantes mesmo
a vazio, pois dependem de $B_{max}$ e $f$, e não da corrente de carga.
""")
        st.markdown('</div>', unsafe_allow_html=True)
